"""
Heuristic v5a — Quantum SPS + Refine Mapping (Step 2a) + Phases 2-6.

Phase 1  : Quantum SPS mapping (mandatory segments, f_max)
Phase 1b : Refine Mapping 2a — linear doublet scan to balance utility density
...
Refine 2a balances processor utility-density UD(x) = Σ u_i*(e_opt_i/p_i)
by iterating all job pairs sorted by ascending |ud_a - ud_b| and swapping
whenever the pair-level UD imbalance decreases and DBF remains feasible.
"""
...
def run(processors, tasks, B_BUDGET):
    ...
    print_instance_summary(processors, tasks, B_BUDGET, h, quantum, N_job, N_seg,
                           ALPHA, BETA, label="USRT Heuristic v5a  —  Instance")
    ...
    # ── Phase 1b: Refine Mapping 2a ──────────────────────────────────────────
    print(f"\n{_S}")
    print(f"  PHASE 1b: REFINE MAPPING — Step 2a (linear doublet scan)")
    print(f"  ud(i) = u_i * e_opt_total_i / p_i  |  UD(x) = sum over jobs on x")
    print(_S2)
    mapping = refine_mapping_2a(mapping, tasks, N_prc, cum, N_seg,
    ...
    tot_e, tot_u = print_schedule(
        ...
        mapping, B_BUDGET, label="HEURISTIC v5a SOLUTION (FINAL)")
