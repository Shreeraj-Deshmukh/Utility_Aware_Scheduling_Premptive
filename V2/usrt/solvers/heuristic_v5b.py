"""
Heuristic v5b — Quantum SPS + Refine Mapping (Step 2b) + Phases 2-6.

Phase 1  : Quantum SPS mapping (mandatory segments, f_max)
Phase 1b : Refine Mapping 2b — one-swap-one-scan to balance utility density
Phase 2  : Left-shift diagnostic
Phase 3  : Energy slack check
Phase 4  : Aggressive frequency scaling
Phase 5  : Greedy optional segment scheduling (DBF window-slack)
Phase 6  : Best-first pairwise segment swap + fill pass

Refine 2b balances processor utility-density UD(x) = Σ u_i*(e_opt_i/p_i)
by repeatedly finding the highest-UD processor P1, scanning doublets for
the first pair whose higher-UD job is on P1, and swapping it with the
lower-UD job — provided max UD strictly decreases and DBF stays feasible.
"""
...
def run(processors, tasks, B_BUDGET):
    ...
    print_instance_summary(processors, tasks, B_BUDGET, h, quantum, N_job, N_seg,
                           ALPHA, BETA, label="USRT Heuristic v5b  —  Instance")
    ...
    # ── Phase 1b: Refine Mapping 2b ──────────────────────────────────────────
    print(f"\n{_S}")
    print(f"  PHASE 1b: REFINE MAPPING — Step 2b (one-swap-one-scan)")
    print(f"  ud(i) = u_i * e_opt_total_i / p_i  |  UD(x) = sum over jobs on x")
    print(_S2)
    mapping = refine_mapping_2b(mapping, tasks, N_prc, cum, N_seg,
                                job_r, job_d, verbose=True)
    proc_jobs, proc_jobs_map = build_proc_jobs(mapping)
    ...
    tot_e, tot_u = print_schedule(
        ...
        mapping, B_BUDGET, label="HEURISTIC v5b SOLUTION (FINAL)")
